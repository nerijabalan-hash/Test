...
export function SectionItem({
  ...
  const openAddSlideDrawer = (mode: 'ai' | 'library' | 'library-search' | 'library-fixed') => {
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            ...
            <div id={sectionBodyId} style={{ padding: 16 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#464646', marginBottom: 6 }}>Section title</div>
                ...
                <div>
                  ...
                  {/* Content area */}
                  <div
                    role={section.slides.length === 0 ? 'button' : undefined}
                    tabIndex={section.slides.length === 0 ? 0 : undefined}
                    onClick={section.slides.length === 0 ? () => openAddSlideDrawer('ai') : undefined}
                    onKeyDown={section.slides.length === 0 ? (e) => {
                    ...
                    style={{
                      background: section.slides.length === 0 ? '#FAFAFA' : '#F3F3F3',
                      border: section.slides.length === 0 ? '1.5px dashed #DFDFDF' : 'none',
                      borderRadius: 10,
                      padding: section.slides.length > 0 ? 6 : 16,
                      minHeight: section.slides.length > 0 ? 'auto' : 80,
                      display: section.slides.length === 0 ? 'flex' : 'block',
                      flexDirection: section.slides.length === 0 ? 'column' : undefined,
                      alignItems: 'center',
                      justifyContent: 'center',
                      cursor: section.slides.length === 0 ? 'pointer' : 'default',
                      transition: 'background 0.15s ease, border-color 0.15s ease',
                    }}
                    onMouseEnter={section.slides.length === 0 ? (e) => {
                      e.currentTarget.style.background = '#F5F5F5';
                      e.currentTarget.style.borderColor = '#BABABA';
                    } : undefined}
                    onMouseLeave={section.slides.length === 0 ? (e) => {
                  ...
                  >
                    {section.slides.length === 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                        <div style={{ fontSize: 13, color: '#737373' }}>Add content to this section</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <IconSparkle size={14} color="#4A61ED" />
                            <span style={{ fontSize: 11, color: '#898989' }}>AI slides</span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <IconPresentation size={14} color="#677848" />
                            <span style={{ fontSize: 11, color: '#898989' }}>Premade slides</span>
                          </div>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        >
                          {section.slides.map((slide) => (
                            <SortableList.Item key={slide.id} id={slide.id} style={{ border: 0, padding: 0, background: 'transparent', boxShadow: 'none' }}>
                              <ContentItemRow
                                slide={slide}
                        ...
                        {/* Add content button inside the content card */}
                        <div style={{ marginTop: 4 }}>
                          <button
                            onClick={() => openAddSlideDrawer('ai')}
